...
forest_spawns = [["Elf", 0, 20, 10, 5, 20],["Golbin", 0, 5, 5, 0, 5],["Harry the Benson", 0, 20, 100, 0, 999]]
deep_forest_spawns = [["Dark Elf", 0, 25, 15, 10, 25]]
swamp_spawns = [["Witch", 0, 25, 25, 10, 30]]
...
def battlefield_selection(): #Selection area for which Battlefield to go to
    global battle_field
    global location_check
    global select_pointer_location
    select_pointer_location = 0
...
def went_direction_check(): #It checks whcih direction you were facing, and then updates the button names and functions accordingly
    ...
    if direction_in_town == 0:
        ...
        add_text("Currently at the Town Entrance")
        add_text("")
        left_button.configure(text = "Hunting Grounds", command = battlefield_selection)
