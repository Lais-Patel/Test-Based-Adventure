...
def wishing_well():
    ...
    def toss_coin():
        encounter_text.delete("6.0","end")
        add_text("")
        add_text("You throw a coin into the fountain, it sinking")
        add_text("to the bottom, ")
...
def gui(): #Making the base for the gui with no functions yet
    ...
    window = tk.Tk()
    window.geometry("470x350") #Sets the size of the window
    window.minsize(470,350)    #Makes it so you can't resize the window
    ...
    health_label= tk.Label(window, text="HP - ", bd=1)
    ...
    health_label.grid(row=2, column=0, columnspan=1,sticky="w",padx=20,pady=10)
    gold_label.grid(row=2, column=1, columnspan=1,sticky="w",padx=20,pady=10)
